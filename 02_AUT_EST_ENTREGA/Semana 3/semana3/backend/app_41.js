...
app.post('/insereexperiencias', urlencodedParser, (req, res) => {
	...
	res.write('<p>EXPERIÊNCIA INSERIDO COM SUCESSO!</p><a href="/">VOLTAR</a>');
	db.close(); // Fecha o banco
	res.end();
});

// Monta o formulário para o update (é o U do CRUD - Update)
app.get('/atualizaexperiencias', (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
...
app.post('/atualizaexperiencias', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = `UPDATE experiencias SET ID =${req.body.ID}, experiência = '${req.body.experiencia}', Data =  '${req.body.Data}' WHERE ID = '${req.body.ID}'`;
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
	...
	res.write('<p>EXPERIÊNCIA ATUALIZADO COM SUCESSO!</p><a href="/">VOLTAR</a>');
	db.close(); // Fecha o banco
});
